...
import { Text, VStack, Box, IconButton, HStack, Icon, Input, TextArea, ScrollView, Center, Checkbox, Slider } from "native-base";
...
export const EventForm = () => {
    ...
    const [strCapacity, setStrCapacity] = useState("Unlimited")
    ...
    function setProfile(profile) {
        setEventPic(profile.neweventpic_url)
    }
    ...
    function checkSlider() {
        if (strCapacity != "Unlimited") {
            return <Slider defaultValue={50} colorScheme="orange" onChange={v => {
                setStrCapacity(Math.floor(v).toString());
  ...
  const handleCreateEvent = async (e) => {
      if (title == "") {
        ...
        // TO BE COMPLETED
        // Useful information:
        // startDate, endDate, eventPic, title, description, location, catString, capacity
